...
-- 3a
update Cruise set price= ROUND(price + price * 0.15, 2)
where departurePort= 'Miami';

select cruiseName, departurePort, shipName, to_char(price, '$9,999.99') AS NEWPRICE
from Cruise;
--where departurePort= 'Miami';

-- 3b
select DISTINCT t3.companyName, t3.cruiseName
from Customer t1 join Reservation t2
on t1.customerID = t2.customerID
join Cruise t3
on t3.cruiseID = t2.cruiseID
where t1.age > 50 AND t3.departurePort= 'Miami'
order by t3.companyName;

-- 3c
select companyName || ' ' || stockSymbol AS CruiseCompany, cruiseName, travelDate
from Company t4 NATURAL JOIN Cruise t5 NATURAL JOIN Reservation t6
where travelDate > '31-DEC-2018' AND days >= 10
order by cruiseName, travelDate;

-- 3d
select t7.firstName, t7.lastName, t7.title
from Reservation t8 Join Customer t9
on t8.customerID = t9.customerID
right join TravelAgent t7
on t7.travelAgentID = t8.travelAgentID
where t8.travelAgentID is Null;